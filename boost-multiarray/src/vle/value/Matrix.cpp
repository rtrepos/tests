...
Matrix::Matrix(index columns, index rows, index columnmax, index rowmax, index
    ...
    : m_extents(), m_matrix(m_extents[columnmax][rowmax]), m_nbcol(columns),
      m_nbrow(rows), m_stepcol(resizeColumns), m_steprow(resizeRows), m_lastX(0),
      m_lastY(0)
{
...
Matrix::Matrix(const Matrix& m)
    : Value(m), m_extents(), m_nbcol(m.m_nbcol), m_nbrow(m.m_nbrow),
      m_stepcol(m.m_stepcol), m_steprow(m.m_steprow), m_lastX(0), m_lastY(0)
{
